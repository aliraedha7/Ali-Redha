...
// Auto-detect bundled assets from dist/assets or assets/ directory
$jsFile = './assets/index-BCkeGgOK.js';
$cssFile = './assets/index-ehU4IgYs.css';

$possibleDirs = [__DIR__ . '/assets', __DIR__ . '/dist/assets'];
foreach ($possibleDirs as $assetsDir) {
    if (is_dir($assetsDir)) {
        $scanned = scandir($assetsDir);
        foreach ($scanned as $f) {
            if (substr($f, -3) === '.js' && strpos($f, 'index-') === 0) {
                $jsFile = (strpos($assetsDir, 'dist') !== false ? './dist/assets/' : './assets/') . $f;
            } elseif (substr($f, -4) === '.css' && strpos($f, 'index-') === 0) {
                $cssFile = (strpos($assetsDir, 'dist') !== false ? './dist/assets/' : './assets/') . $f;
            }
        }
    }
}
